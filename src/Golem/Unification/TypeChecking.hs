...
module Golem.Unification.TypeChecking where

import Golem.Utils.ABT
import Golem.Utils.Elaborator
import Golem.Utils.Eval
...
evaluate (SubstitutedTerm m) =
  do defs <- getElab definitions
     ...
     case runReaderT (paramEval l m) (definitionsToEnvironment defs) of
       Left e   -> throwError e
...
-- | We can modify the quote level we check at by adding a quote level.

addQuoteLevel :: TypeChecker a -> TypeChecker a
addQuoteLevel tc =
  do l <- getElab quoteLevel
     putElab quoteLevel (l+1)
     x <- tc
     putElab quoteLevel l
     return x


-- | We can modify the quote level we check at by removing a quote level.

removeQuoteLevel :: TypeChecker a -> TypeChecker a
removeQuoteLevel tc =
  do l <- getElab quoteLevel
     putElab quoteLevel (l-1)
     x <- tc
...
extendResets newResets tc =
  do resets <- getElab resetPointsInScope
     putElab resetPointsInScope (newResets ++ resets)
     x <- tc
     putElab resetPointsInScope resets
     return x


-- | We can get the types of a reset point.

getResetPoint :: String -> Elaborator (Term,Term)
getResetPoint res =
  do resets <- getElab resetPoints
     case lookup res resets of
       Nothing ->
         throwError $ "The reset point " ++ res ++ " has not been declared."
       Just (lower,upper) -> return (lower,upper)


-- | We can add a shifted reset point by pushing it onto the in scope shift
-- stack, and zeroing out the context temporarilly.

addShift :: String -> Elaborator a -> Elaborator a
addShift res tc =
...
-- | We can remove a shift by popping a the top shift from the shift stack.

removeShift :: Elaborator a -> Elaborator a
removeShift tc =
  do shifts <- getElab shiftsInScope
     case shifts of
       [] -> tc
       _:shifts' ->
         do putElab shiftsInScope shifts'
            x <- tc
            putElab shiftsInScope shifts
            return x
...
--
--      Γ ; K' ⊢ A ▹ A' ⇐ Type at l    Resets ∋ Ki
--    ----------------------------------------------- quoted
--    Γ ; K' ⊢ Quoted[K] A ▹ Quoted[K] A' ⇒ Type at l
--
--    Γ ; K' ⊢ M ▹ M' ⇒ Quoted[K] A at l-1    K ⊆ K'
--    ---------------------------------------------- unquote
--              Γ ; K' ⊢ ~M ▹ ~M' ⇒ A at l
--
--    Resets ∋ s : A resets B    Γ ; S ⊢ M ▹ M' ⇐ A at l+1
--    ---------------------------------------------------- continue
...
--
--    Resets ∋ k : A resets B    Γ ; K, k ⊢ M ▹ M' ⇐ B at l+1
--    ------------------------------------------------------- reset
--        Γ ; K ⊢ reset k in M ▹ reset k in M' ⇒ B at l+1
-- @
--
...
inferify (In (Unquote m)) =
  do (ElaboratedTerm elm, t) <- removeQuoteLevel $ inferify (instantiate0 m)
     ...
     case et of
       In (QuotedType res a) ->
         do resets <- getElab resetPointsInScope
            unless (all (`elem` resets) res)
              $ throwError $ "Reset points mismatch for the term "
                          ++ pretty (instantiate0 m)
                          ++ " which has points " ++ unwords res
                          ++ " and the context which has "
                          ++ unwords resets
  ...
  do l <- getElab quoteLevel
     ...
     case shifts of
       [] -> throwError $ "Cannot continue with " ++ pretty (instantiate0 m)
               ++ " because there are no shifted reset points in scope."
       res:_ ->
...
--         Γ ; K ⊢ M ▹ M' ⇐ A at l+1
--    ------------------------------------ quote
--    Γ ; K' ⊢ `M ▹ `M' ⇐ Quoted[K] A at l
--
--    Γ ⊢ A ▹ A'  ⇐ Type at l+1   Γ, x : A' at l+1 ⊢ M ▹ M'  ⇐ B at l+1
--    ----------------------------------------------------------------- req
--        Γ ⊢ require x : A in M ▹ require x : A' in M' ⇐ B at l+1
-- @
       ...
       case (plic,t) of
         (Expl, In (Fun Expl a sc2)) ->  -- \x -> M : (x : A) -> B
           do [v@(FreeVar n)] <- freshRelTo (names sc) context
              ...
              ElaboratedTerm m'
                <- do l <- getElab quoteLevel
                      extendElab context [(v,QLJ (instantiate0 a) l)]
  ...
  where
    ...
    go _ _ _ =
      error $ "This case of 'go' in 'checkify' of a record type should "
           ++ "be unreachable."
checkify (In (Quote m)) (NormalTerm t) =
  case t of
    In (QuotedType res a) ->
      do ElaboratedTerm em <-
           addQuoteLevel
             $ extendResets res
               $ checkify (instantiate0 m)
                          (NormalTerm (instantiate0 a))
...
checkify (In (Require a sc)) (NormalTerm t) =
  do l <- getElab quoteLevel
     ...
     [x@(FreeVar n)] <- freshRelTo (names sc) context
     ElaboratedTerm em <-
       extendElab context [(x,QLJ ea l)]
         $ checkify (instantiate sc [Var (Free x)])
                    (NormalTerm t)
     return $ ElaboratedTerm (requireH n ea em)
checkify m t =
...
checkifyTelescope (Telescope ascs) =
  ...
  where
    ...
    go vars acc (sc:scs) =
      do l <- getElab quoteLevel
         let ctx' = zipWith (\x y -> (x,QLJ y l)) vars acc
